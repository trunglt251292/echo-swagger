...
type Api interface {
	// Response Default
	ResponseDefault(res []*ResponseJSON) Api
	// Add multi param in query
	InQuery(params []*Params) Api
	// Add multi param in body
	InBody(params *Params) Api
	// Add Multi Param In Path
	InPath(params []*Params) Api
	// Add Description and Summary
	AddDescriptions(desc *DescriptionAPI) Api
...
func (a *api) AddDescriptions(desc *DescriptionAPI) Api {
	if desc.Description != "" {
		a.SetDescription(desc.Description)
	}
	if desc.Summary != "" {
		a.SetSummary(desc.Summary)
	}
	return a
}
...
func (a *api) InPath(params []*Params) Api {
	for _, param := range params {
		a.addParams(param.Param, ParamInPath, param.Name, param.Desc, param.Required, false)
	}
	return a
}

func (a *api) InBody(param *Params) Api {
	a.addBodyParams(param.Param, param.Name, param.Desc, param.Required)
	return a
}

func (a *api) InQuery(params []*Params) Api {
	for _, param := range params {
		a.addParams(param.Param, ParamInQuery, param.Name, param.Desc, param.Required, false)
	}
	return a
}

func (a *api) ResponseDefault(res []*ResponseJSON) Api {
	for _, param := range res {
		a.AddResponse(param.Code, param.Desc, param.Schema, param.Header)
	}
	return a
}
